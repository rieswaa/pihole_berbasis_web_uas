...
// Tambah domain ke blacklist
if (isset($_POST['domain'])) {
    $domain = mysqli_real_escape_string($conn, $_POST['domain']);
    mysqli_query($conn, "INSERT INTO blacklist (domain) VALUES ('$domain')");
    header("Location: blacklist.php");
    exit;
}

// Hapus domain dari blacklist
if (isset($_GET['delete'])) {
    $id = intval($_GET['delete']);
    mysqli_query($conn, "DELETE FROM blacklist WHERE id = $id");
    header("Location: blacklist.php");
    exit;
}

$result = mysqli_query($conn, "SELECT * FROM blacklist ORDER BY id DESC");
?>
<!DOCTYPE html>
<html lang="id">
<head>
